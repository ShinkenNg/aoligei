...
const CheckboxList: React.FC<{
  list: (PowerColumns<any> & { index?: number })[];
  className?: string;
  title: string;
  showTitle?: boolean;
...
const GroupCheckboxList: React.FC<{
  localColumns: (PowerColumns<any> & { index?: number })[];
  className?: string;
}> = ({ localColumns, className }) => {
  const rightList: (PowerColumns<any> & { index?: number })[] = [];
  const leftList: (PowerColumns<any> & { index?: number })[] = [];
  const list: (PowerColumns<any> & { index?: number })[] = [];
  const intl = useIntl();
...
const ColumnSetting = <T, U = {}>(props: ColumnSettingProps<T>) => {
  ...
  const localColumns: Omit<PowerColumns<any> & { index?: number }, 'ellipsis'>[] =
